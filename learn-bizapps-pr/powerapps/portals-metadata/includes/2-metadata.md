...
| Website Bindings | Website bindings link the particular website to the Azure portal web application. Website bindings can be created, edited, and deleted within Power Apps portals. |
| Settings | Similar to settings, the Settings table contains value pairs that are specific to all portals that are provisioned for the particular environment as opposed to a specific portal. |
...
| Table Forms | Table forms use model-driven form definitions to display forms in the portal and enable access to Dataverse records for portal visitors. An table form can be configured to display read-only, edit, or create forms. |
| Table Lists | Table lists use model-driven view definitions to display lists of Dataverse records. An table list can be configured to allow navigation to a selected record or to run workflows. A maker can also enable advanced search and filter capabilities. |
...
| Web Forms | Web forms are components similar to table forms, but instead of a single record/form, web forms create a sequence of steps that can be used to build a portal-based business process flow. Web forms can currently only be configured by using the Portal Management app. |
...
| Web Pages | The webpage record is the core of the website. It defines the actual displayed content, both static and through other components like table lists and table forms. A webpage can be created and edited by using Power Apps portals Studio and the Portal Management app. |
...
| Metadata | Purpose |
| --- | --- |
| Contacts | The contact table is the actual Dataverse or Dynamics 365 contact table. All authenticated portal visitors, including internal staff members, must have a corresponding contact record regardless of the authentication method. |
| Table Permissions | Table permissions define the scope and the privileges for specific Dataverse records. Each table permission record can be associated with a web role, granting configured data access to portal visitors. |
...
| Web Roles | One or many web role records can be assigned to a portal user (contact). The web role can be linked to table permissions and Web Page Access Control Rules to control access to content and Dataverse records.|
