There are other DAX time intelligence functions, which are concerned with returning a single date. Let's introduce these functions by applying them in two different scenarios.
